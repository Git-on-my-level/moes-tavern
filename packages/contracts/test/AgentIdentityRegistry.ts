...
describe("AgentIdentityRegistry", function () {
  ...
  it("allows approved operators to update agentURI", async function () {
    ...
    await expect(registry.connect(operator).setAgentURI(1, "ipfs://agent-1-updated"))
  ...
  it("rejects non-owner updates", async function () {
    ...
    await expect(registry.connect(other).setAgentURI(1, "ipfs://bad")).to.be.revertedWith(
      "AgentIdentityRegistry: not authorized"
    );
