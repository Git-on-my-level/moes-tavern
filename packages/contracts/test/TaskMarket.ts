...
describe("TaskMarket", function () {
  async function deployFixture() {
    ...
    return { owner, buyer, agent, other, identity, listingRegistry, taskMarket, disputeModule, token };
  }
  ...
  it("enforces buyer-only dispute opening and resolver-only resolution", async function () {
    const { buyer, agent, other, listingRegistry, taskMarket, disputeModule, token } = await deployFixture();
    await createListing(listingRegistry, agent, token.target, { quoteRequired: false });
    ...
    await expect(disputeModule.connect(other).openDispute(1, "ipfs://dispute-1")).to.be.revertedWith(
      "DisputeModule: buyer only"
    );
  ...
  it("splits escrow deterministically on SPLIT outcome", async function () {
    const { buyer, agent, listingRegistry, taskMarket, disputeModule, token, owner } = await deployFixture();
    await createListing(listingRegistry, agent, token.target, { quoteRequired: false });
    ...
    await disputeModule.connect(owner).resolveDispute(1, 2, "ipfs://resolution-2");
  ...
  it("slashes bond up to the locked amount on BUYER_WINS", async function () {
    const { buyer, agent, listingRegistry, taskMarket, disputeModule, token, owner } = await deployFixture();
    const { policy } = await createListing(listingRegistry, agent, token.target, { quoteRequired: false }, { sellerBondBps: 5000 });
    ...
    const task = await taskMarket.getTask(1);
    const requiredBond = (task.quotedTotalPrice * BigInt(policy.sellerBondBps)) / 10_000n;
    ...
    await disputeModule.connect(owner).resolveDispute(1, 1, "ipfs://resolution-3");
