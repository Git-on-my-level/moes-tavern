...
import type { Indexer as IndexerClass, IndexerEvent, ListingCuration } from "../../indexer/src/indexer";
...
const dynamicImport = async <T>(modulePath: string): Promise<T> => {
  const importer = new Function("href", "return import(href);") as (href: string) => Promise<T>;
...
async function main() {
  ...
  const { checkEndpoints, computeCurationBadges, computeRiskScore, lintListingMetadata, runProbeFixture } =
    await dynamicImport<CurationModule>(path.resolve(__dirname, "../../curation/src/curation.ts"));
