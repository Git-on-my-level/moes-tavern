...
import type {
  Indexer as IndexerClass,
  IndexerEvent,
  ListingCuration,
} from '../../indexer/src/indexer';
...
const listingMetadataById = new Map<
  number,
...
>([
  [
    1,
    {
      title: 'Agent build assistant',
      description: 'Prototype build + QA support',
      tags: ['ai', 'dev'],
    },
  ],
  [
    ...
    {
      title: 'Prompt review',
      description: 'Review prompts for clarity',
      tags: ['review', 'prompt'],
    },
  ],
]);
...
const dynamicImport = async <T>(modulePath: string): Promise<T> => {
  const importer = new Function('href', 'return import(href);') as (
    href: string,
  ) => Promise<T>;
  return importer(pathToFileURL(modulePath).href);
};

async function main() {
  const { Indexer } = await dynamicImport<{ Indexer: typeof IndexerClass }>(
    path.resolve(__dirname, '../../indexer/src/indexer.ts'),
  );
  type CurationModule = typeof import('../../curation/src/curation');
  const {
    checkEndpoints,
    ...
    lintListingMetadata,
    runProbeFixture,
  } = await dynamicImport<CurationModule>(
    path.resolve(__dirname, '../../curation/src/curation.ts'),
  );

  const [, buyer, agent, other] = await ethers.getSigners();
  ...
  await listingRegistry
    .connect(agent)
    .createListing(1, 'ipfs://listing-1', pricingBase, policyBase);
  await listingRegistry
    .connect(agent)
    .createListing(
      1,
      'ipfs://listing-2',
      { ...pricingBase, quoteRequired: false },
      policyBase,
    );
  ...
  const settledLogs = await taskMarket.queryFilter(
    taskMarket.filters.TaskSettled(1),
  );
  ...
  assert.equal(
    buyerBalance,
    10_000n - quotedTotalPrice,
    'buyer balance should reflect escrow payout',
  );
  ...
  const listingCreatedLogs = await listingRegistry.queryFilter(
    listingRegistry.filters.ListingCreated(),
  );
  ...
  const listingUpdatedLogs = await listingRegistry.queryFilter(
    listingRegistry.filters.ListingUpdated(),
  );
  ...
  const taskPostedLogs = await taskMarket.queryFilter(
    taskMarket.filters.TaskPosted(),
  );
  ...
  const quoteProposedLogs = await taskMarket.queryFilter(
    taskMarket.filters.QuoteProposed(),
  );
  ...
  const quoteAcceptedLogs = await taskMarket.queryFilter(
    taskMarket.filters.QuoteAccepted(),
  );
  ...
  const taskFundedLogs = await taskMarket.queryFilter(
    taskMarket.filters.TaskFunded(),
  );
  ...
  const taskAcceptedLogs = await taskMarket.queryFilter(
    taskMarket.filters.TaskAccepted(),
  );
  ...
  const deliverableLogs = await taskMarket.queryFilter(
    taskMarket.filters.DeliverableSubmitted(),
  );
  ...
  const settledTaskLogs = await taskMarket.queryFilter(
    taskMarket.filters.TaskSettled(),
  );
  ...
  const nowMs =
    (latestBlock?.timestamp ?? Math.floor(Date.now() / 1000)) * 1000;
  const probeFixturePath = path.resolve(
    __dirname,
    '../../curation/test/fixtures/probe-pass.json',
  );
  ...
  for (const listing of listings) {
    ...
    const badges = computeCurationBadges(
      lint,
      endpointReport,
      probe.probeScore,
    );
  ...
  assert.ok(
    curatedListings.every(
      (listing) => listing.curation && listing.curation.badges,
    ),
